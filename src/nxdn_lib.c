...
void NXDN_SACCH_Full_decode(dsd_opts * opts, dsd_state * state)
{
  ...
  for(i = 0; i < 4; i++)
  {
    ...
    //fprintf(stderr, "\n  SAACH P# %1d - %05X", i, state->NxdnSacchRawPart[i].Data);
  }
                                  ...
                                  uint8_t CrcCorrect, uint8_t * ElementsContent)
{
  ...
  //fprintf(stderr, "\n  Message Type = 0x%02X ", MessageType & 0xFF);

  //Dump the Message into a HEX value for vieweing
  //FullMessage = (unsigned long long int)ConvertBitIntoBytes(&Message[0], 64);
  for (short int i = 0; i < 64; i++)
  {
    FullMessage = FullMessage << 1;
    FullMessage |= ElementsContent[i];
  }
