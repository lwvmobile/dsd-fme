...
namespace {
    static dsd_opts *copy_opts(const dsd_opts *src) {
...
int RtlSdrOrchestrator::start() {
    if (started_) return 0;
    if (!opts_) { last_error_code_ = -1; return last_error_code_; }
    int r = dsd_rtl_stream_open(opts_);
    if (r < 0) { last_error_code_ = r; return r; }
    started_ = true;
    last_error_code_ = 0;
    return 0;
}

int RtlSdrOrchestrator::stop() {
    if (!started_) return 0;
...
int RtlSdrOrchestrator::tune(uint32_t center_freq_hz) {
    if (!started_) { last_error_code_ = -1; return last_error_code_; }
    if (!opts_) { last_error_code_ = -2; return last_error_code_; }
    dsd_rtl_stream_tune(opts_, (long int)center_freq_hz);
    last_error_code_ = 0;
    return 0;
}

int RtlSdrOrchestrator::read(int16_t *out, size_t count, int &out_got) {
    if (!started_) { last_error_code_ = -1; return last_error_code_; }
    if (!opts_) { last_error_code_ = -2; return last_error_code_; }
    int got = dsd_rtl_stream_read(out, count, opts_, (dsd_state*)nullptr);
    if (got < 0) { last_error_code_ = got; return got; }
    out_got = got;
    last_error_code_ = 0;
    return 0;
}

unsigned int RtlSdrOrchestrator::output_rate() const {
