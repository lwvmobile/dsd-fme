...
extern "C" int rtl_stream_create(const dsd_opts* opts, RtlSdrContext** out_ctx) {
    if (!out_ctx || !opts) return -1;
...
extern "C" int rtl_stream_start(RtlSdrContext* ctx) {
    if (!ctx || !ctx->stream) return -1;
    return ctx->stream->start();
}

extern "C" int rtl_stream_stop(RtlSdrContext* ctx) {
    if (!ctx || !ctx->stream) return -1;
    return ctx->stream->stop();
}

extern "C" int rtl_stream_destroy(RtlSdrContext* ctx) {
    if (!ctx) return 0;
    if (ctx->stream) {
        ctx->stream->stop();
        delete ctx->stream;
...
extern "C" int rtl_stream_tune(RtlSdrContext* ctx, uint32_t center_freq_hz) {
    if (!ctx || !ctx->stream) return -1;
    return ctx->stream->tune(center_freq_hz);
}

extern "C" int rtl_stream_read(RtlSdrContext* ctx, int16_t* out, size_t count, int* out_got) {
    if (!ctx || !ctx->stream || !out || !out_got) return -1;
    return ctx->stream->read(out, count, *out_got);
}

extern "C" uint32_t rtl_stream_output_rate(const RtlSdrContext* ctx) {
    if (!ctx || !ctx->stream) return 0U;
    return ctx->stream->output_rate();
}

extern "C" void rtl_stream_clear_output(RtlSdrContext* /*ctx*/) {
    // Delegate to legacy function to keep behavior identical
    dsd_rtl_stream_clear_output();
}

extern "C" long rtl_stream_return_pwr(const RtlSdrContext* /*ctx*/) {
