...
main (int argc, char **argv)
{
  ...
  while ((c = getopt (argc, argv, "haep:P:qstv:z:i:o:d:c:g:nwB:C:R:f:m:u:x:A:S:M:G:D:L:V:U:Y:K:NWrlZ")) != -1)
    {
      opterr = 0;
      switch (c)
        {
        ...
        case 'W': //monitor_input_audio if no sync
          ...
          fprintf (stderr,"Monitor Source Audio Disabled in Cygwin Builds.\n");
          //opts.pulse_raw_rate_in   = 48000;
          //opts.pulse_raw_rate_out  = 48000; //doing tests with 2 channels at 24000 for 48000 audio default in pulse
          //opts.pulse_digi_rate_in  = 48000;
          //opts.pulse_digi_rate_out = 48000; //need to copy this to rtl type in and change rate out to 8000
        ...
        case 'd':
		  ...
		  opts.audio_out_type = 0; //hack for cygwin so it will open the damn pulse audio output
          break;
        case 'c':
        ...
        case 'w':
		  ...
		  opts.audio_out_type = 0; //trick to open anyways in case user specifies a -w in cygwin?
		  fprintf (stderr,"Writing + Appending WAV files currently disabled in Cygwin builds!\n");
          break;
        case 'B':
	...
	if((strncmp(opts.audio_in_dev, "/dev/dsp", 5) == 0))
    {
      opts.audio_in_type == 0;
    }

    //if((strncmp(opts.audio_out_dev, "pulse", 5) == 0))
	if((strncmp(opts.audio_out_dev, "/dev/dsp", 5) == 0))
    {
